...
if ( $bgcolor == "custom" && $custombgcolor != '' ) {
	$custombgcolor = ' style="' . vc_get_css_color( 'background', $custombgcolor ) . '"';
	$bgcolor = "";
}
if ( $bgcolor != "" ) $bgcolor = " " . $bgcolor;

$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'vc_progress_bar skills-section wpb_content_element' . $el_class, $this->settings['base'], $atts );
...
$bgcolor	=	empty( $bgcolor ) ? 'default' : esc_attr( $bgcolor );
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '
		<p>'.$line['label'].' <span>'.$line['value'].'%</span></p>
		<div class="meter nostrips frontend">
			<span class="'.$bgcolor.'" style="width: '.$line['value'].'%"></span>
		</div>
	';	
}
/**
