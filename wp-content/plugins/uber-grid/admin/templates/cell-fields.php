		...
		<div class="column">
			...
			<div class="field">
				...
				<?php else: ?>
						...
						<label><?php _e('Show tags', 'uber-grid') ?></label>
						<select name="<?php echo $name_prefix ?>[tags_mode]">
							<option value="" <?php selected('', $cell->tags_mode) ?>><?php _e('All the tags', 'uber-grid') ?></option>
							<option value="leaf" <?php selected('leaf', $cell->tags_mode) ?>><?php _e('Leaf tags only', 'uber-grid') ?></option>
						</select>
				<?php endif ?>
			</div>
		</div>
